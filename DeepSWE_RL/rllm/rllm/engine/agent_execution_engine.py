...
from rllm.engine.garbled_text_file import detect_garbled_text
...
class AgentExecutionEngine:
    def __init__(
        ...
        total_timeout_llm_env_reward = None,
        gamma=0.2,
        api_retries=3,
        retry_limit=3,
        max_steps=5,
        ...
        **kwargs,
        ...
        if not trajectory_timeout:
            self.trajectory_timeout = int(4200)
            # self.trajectory_timeout = int(5400)
        if not env_response_timeout:
            self.env_response_timeout = int(400)
        if not total_timeout_llm_env_reward:
            self.total_timeout_llm_env_reward = int(5400)
            # self.total_timeout_llm_env_reward = int(6800)
        if not env_response_retry_num:
            self.env_response_retry_num = int(1)
    ...
    async def run_agent_trajectory_async(self, idx, application_id, seed=0, mode="Text", **kwargs):
        ...
        time_env_reset = time.time()
        observation, info = await loop.run_in_executor(self.executor, env.reset)
        time_env_reset = time.time() - time_env_reset
        # print(f"!!!!!!! env reset time: {time_env_reset}")
        ...
        garbled_terminated = False  # [NEW] 标记是否因乱码提前终止（用于跳过 final reward）
        ...
        for step_idx in range(self.max_steps):
            ...
            response = await self.get_model_response(prompt_messages, application_id, **kwargs) # 内部的router设置了三次retry
            ...
            # ================================
            # [NEW] 乱码检测：一旦检测到，立刻终止整条轨迹并 reward=-1
            # ================================
            try:
                is_garbled = detect_garbled_text(response, return_debug=False)
            except Exception:
                is_garbled = False
            if is_garbled:
                termination_reason = "GARBLED_OUTPUT"
                reward = -0.5
                garbled_terminated = True
                print(f"detect garbled text")

                # 把 assistant 输出写入 chat_completions，保证轨迹完整
                # chat_completions_messages = agent.chat_completions
                assistant_message = agent.chat_completions[-1]
                # chat_completions_messages.append(assistant_message)

                # 记录 token 版本轨迹（只加 assistant，不再继续 env step）
            ...
            for retry_num_get_env_response in range(self.env_response_retry_num):
                try:
                    ...
                    print(f"Attention!!! 得到env response的时候timeout了！！这是第{retry_num_get_env_response+1}次尝试，共能尝试{self.env_response_retry_num}次")
                    next_observation = "The environment timed out. This is not your fault and please continue trying."
                    reward = 0
                    done = False
                    info = {"total_time": self.env_response_timeout}
                    flag_get_env_response = True
                    continue
            ...
            if not flag_get_env_response:
                ...
                if step_idx == 0:
                    ...
                    print(f"Warning: Trajectory {idx} completed due to: {termination_reason} before able to perform 1 complete action. This might cause unexpected behavior. Consider increasing trajectory timeout limit.\n")
                reward = 0
            ...
            response_token_len += len(assistant_msg_tokens) + len(env_msg_tokens) # 这里是包含了assitant和env的回复的长度
        ...
        if self.overlong_filter:
            if  termination_reason == "ENCODE_ERROR" or termination_reason == "PROMPT_TRUNCATION":
            ...
            if termination_reason =="MAX_STEPS" or termination_reason == "TRAJ_TIMEOUT" or termination_reason == "TRUNCATION":
                reward_zero = True
            
        # 默认的reward是0，所以被masked_out后reward就仍然是0
        reward_timeout = 999999999
        # if hasattr(env, "compute_final_reward") and not masked_out and not garbled_terminated:
        if hasattr(env, "compute_final_reward") and not masked_out and not garbled_terminated and not reward_zero:
        # if hasattr(env, "compute_final_reward") :
            cur_step = agent.get_current_state()
            reward_timeout = self.total_timeout_llm_env_reward - total_time
            start_time = time.time()
            # reward = await loop.run_in_executor(self.executor, env.compute_final_reward)
            try:
                reward = await asyncio.wait_for(
                    loop.run_in_executor(self.executor, env.compute_final_reward),
                    timeout=min(int(reward_timeout),1200),  # 之前是
                )
            except asyncio.TimeoutError:
                print(f"!!!! reward TimeoutError for env: {idx}")
                # 超时后的兜底逻辑
                termination_reason = "REWARD_TIMEOUT"
                reward = 0.0

            if reward <-10000:
                termination_reason = "REPORT_JSON_LOAD_FAILED"
                reward = 0.0

            agent_patch = env.env.agent_patch # 来自模拟的环境的时候，env.env.agent_patch有值
            reward_context = env.env.get_reward_context()

            if agent_patch is None:
                print(f"!!!!!!agent_patch is None for env: {idx}")
            if reward_context is None:
                print(f"!!!!!!reward_context is None for env: {idx}")

            reward_time = time.time() - start_time
            print(f"!!!!time taken to compute final reward: {reward_time:.2f}")
            if reward_half:
                reward = reward * 0.5
            cur_step.reward = reward

        if self.overlong_filter:
            if termination_reason == "REWARD_TIMEOUT" or termination_reason == "REPORT_JSON_LOAD_FAILED":
                # Mask out the entire response for overlong trajectories if the reward is 0.
                response_masks = [0] * len(response_masks)

        # Closing environment using the executor.
        print(f"now closing env: {idx}")
        time_env_close = time.time()
        await loop.run_in_executor(self.executor, env.close)
        time_env_close = time.time() - time_env_close
        print(f"!!!!time taken to close env: {time_env_close:.2f}")
        ...
        if mode == "Text":
            ...
            #     else:
            #         traj_reward = 0
            #     print(f"Trajectory {idx} is half_reward: {traj_reward}, termination_reasonSTEP: {termination_reason}")
            # elif termination_reason == "TRUNCATION":
            #     traj_reward = -1
            #     print(f"Trajectory {idx} is traj_reward: {traj_reward}, termination_reasonTRU: {termination_reason}")
            if termination_reason == "GARBLED_OUTPUT":
                traj_reward = -0.5
                print(f"Trajectory {idx} is traj_reward: {traj_reward}, termination_reasonGAR: {termination_reason}")
            elif termination_reason =="MAX_STEPS" or termination_reason == "TRAJ_TIMEOUT" or termination_reason == "TRUNCATION":
                traj_reward = 0
            else:
                traj_reward = trajectory.reward
            trajectory.reward = traj_reward
            token_result = {
                ...
                "metrics": {
                    ...
                    "llm_time": llm_time,
                    "env_open_time":time_env_reset,
                    "env_close_time":time_env_close,
                ...
                "timeout_info":
                    {
                        "total_timeout":self.total_timeout_llm_env_reward,
                        "traj_timeout":self.trajectory_timeout,
                        "reward_timeout":min(int(reward_timeout),1200),
                        "single_env_timeout":self.env_response_timeout,
                    }
            }
            return token_result
    ...
    async def trajectory_generator(self, reset_seed=0, timing_raw=None, mode="Text", **kwargs):
        ...
        for coro in asyncio.as_completed(tasks_to_run):
            try:
                result = await coro
                tasks_completed += 1
                print(f"Number of Trajectories {tasks_completed}/{len(self.envs)} completed")
                yield result
            except Exception as e:
                raise e
        ...
        async def sem_wrapper(task_id, task):
            ...
            async with semaphore:
                ...
                try:
                    ...
                    res.task = task
                    completed += 1
                    print(f"Progress: {completed}/{total} trajectories completed")
                    return task_id, res
                finally:
