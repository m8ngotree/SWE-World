...
def delete_path(path: str) -> None:
    try:
        if os.path.isdir(path) and not os.path.islink(path):
            shutil.rmtree(path)
            print(f"shutil Deleted path: {path}")
        else:
            os.remove(path)
            print(f"os.remove Deleted path: {path}")
        
    except Exception:
        print(f"delete_path ERROR: {path}")
...
class AgentPPOTrainer(RayPPOTrainer):
    ...
    def generate_agent_trajectory(self, timing_raw=None, meta_info=None):
        ...
        with _timer("collect_trajectory", timing_raw):
            trajectories = []
            if self.config.agent.async_engine: # 走的这个分支
    ...
    def _transform_agent_trajectories(self, trajectories: list[dict]):
        ...
        for traj in trajectories:
            ...
            agent_patches.append(traj["agent_patch"])
            reward_contexts.append(traj["reward_context"])
            traj_metrics.append(traj["metrics"])
            traj_metrics_ori.append(traj["metrics"])
        if empyt_num == len(trajectories):
            print(f"Ops!!!全挂了！empry num is {empyt_num}")
                ...
                for idx_write in range(sample_num):
                    try:
                        obj_now = {
                            ...
                            "agent_patch":agent_patches[idx_write],
                            "reward_context":reward_contexts[idx_write],
                        ...
                        f.write((agent_patches[idx_write] if agent_patches[idx_write] is not None else "") + "\n")
                        f.write("++++++++++++\n")
                        print(f"写入轨迹失败了!!!!：\n {agent_patches[idx_write]}")
            else:
                print(f"Ops!!! 这个batch存的时候数据没对齐！！这个batch是：{self.global_steps}\nlen(chat_completions):{len(chat_completions)};len(traj_scores):{len(traj_scores)};len(traj_metrics_ori):{len(traj_metrics_ori)}")
