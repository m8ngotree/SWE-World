...
class Agent:
    ...
    def run(
        ...
        # self.logger.info(f"Instance Prompt Template: {self.instance_prompt_template}")
        # self.logger.info(f"System Prompt Template: {self.system_prompt_template}")
        ...
        while not done:
            ...
            for model_gen_try in range(model_gen_max_num):
                try:
                    ...
                    self.logger.error(f"Error querying LLM: {e}. This is No.{model_gen_try} error, you have {model_gen_max_num - model_gen_try-1 } attempts remaining")
            ...
            if "<parameter=command=" in response_content:
                self.logger.warning(f"Ops!!! qwen又把=当成>放到para结尾了!!!")
            ...
            if self.use_fn_calling:
                ...
                thought, action = self.parse_response(assistant_message) #都提取出来fc前面的内容即可，就两个think token不用额外处理
            ...
            elif not model_gen_finished_flag:
                self.logger.info(
                    f"Repeat for requests but failed."
                )
                done = True
        ...
        self.trajectory = Trajectory(
            ...
            docker_image=getattr(env.runtime, 'docker_image', 'simulated'),
            agent_args=asdict(self.args),
            #env_args=asdict(env.args),
            env_args=asdict(env.args) if is_dataclass(env.args) else vars(env.args),
            max_steps=max_steps,
